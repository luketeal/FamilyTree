...
- We own the layout algorithm, including its bugs. The prototype already shows one: couple merging pairs each person with their *first* spouse only — `spouseOf` is a first-wins pairwise map — so a remarriage leaves the second spouse non-adjacent and draws a long marriage edge across the diagram (visible in `results/hand-small.png` and `results/dagre-small.png`). **PR 10 must address it**, and the shape of the fix is worth stating precisely, because it is more tractable than a first look suggests:
  ...
  Measured at 500 people: of 222 marriage edges, **190 are correctly adjacent and 32 are not**, and those 32 correspond exactly to the 32 people with a second marriage. No one in that graph has three, so *every* defect present is the tractable case — the chain fix would clear all of them.

- **Long marriage edges are routed in the worst possible way, which is a separate bug from the one above.** A marriage is drawn as a straight horizontal line at the row's centre — card mid-height — and nodes paint after edges. A non-adjacent marriage therefore threads behind every card in its row and re-emerges between them, reading as one continuous rule spanning the whole generation rather than as a link between two people. At 500 people the longest span 7,677px, and a handful of them make a generation look married to itself.

  This matters even after the adjacency fix, because the residual cases (three or more marriages, marriage cycles) will still produce long edges. PR 10 should route a non-adjacent marriage deliberately — dropping below the row and returning, the way the parent-child edges already step around — so that a long marriage reads as an unusual link rather than as a horizontal rule.
