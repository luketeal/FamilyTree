...
**Only placeholders something still points at are written.** A phantom's entire content is being the far end of a link — no name, no dates, no id anybody has seen — so one with no links left is residue rather than a record, and nothing in the app can reach it to delete it. Removing or replacing an unidentified parent orphans its phantom, and while exports excluded phantoms outright a round trip swept those orphans by accident. Carrying them removed that accidental collection, so the filter has to be deliberate: otherwise every correction leaves a record that survives each later backup and pushes the summary's "N unidentified ancestors" upward with nothing to explain it, on the page whose job is proving the file is intact. Sweeping the phantom when its last link goes would be better still, but that spans two records and so wants a single repository call under this project's own seam rule; the export-side filter keeps the file honest in the meantime.
